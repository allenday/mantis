...
class AbstractNarrator(ABC):
    ...
    def __init__(
        self, execution_strategy: mantis_core_pb2.ExecutionStrategy = mantis_core_pb2.EXECUTION_STRATEGY_DIRECT
    ):
    ...
    @abstractmethod
    async def aggregate_responses(self, team_result: mantis_core_pb2.TeamExecutionResult) -> str:
        """
    ...
    @abstractmethod
    async def create_narrative_context(
        self,
    ...
    async def synthesize_narrative(
        self, simulation_input: mantis_core_pb2.SimulationInput, team_result: mantis_core_pb2.TeamExecutionResult
        ...
        narrative_context = await self.create_narrative_context(simulation_input, team_result, aggregated_responses)
        ...
        execution_context.assigned_role = RolePreference.Name(
            RolePreference.ROLE_PREFERENCE_NARRATOR
        )  # Key role for synthesis
...
class MultiAgentOrchestrator:
    ...
    def __init__(self, team_formation: AbstractTeamFormation, narrator: AbstractNarrator):
...
def create_tarot_orchestrator(
    ...
    return MultiAgentOrchestrator(
        team_formation=TarotTeamFormation(execution_strategy), narrator=TarotNarrator(execution_strategy)
    )
