...
from ..prompt import ContextualPrompt, create_simulation_prompt, create_simulation_prompt_with_interface
...
class MantisService:
    ...
    def __init__(self):
        try:
            ...
            logger.info(
                ...
                structured_data={
                    "version": "1.0",
                    "orchestrator_tools": len(self.orchestrator.get_available_tools())
                }
            )
    ...
    async def process_simulation_input(
        ...
        try:
            # Execute with A2A lifecycle management
            completed_task = await self.orchestrator.execute_task_with_a2a_lifecycle(
                query=simulation_input.query,
                agent_interface=agent_interface,
                context_id=simulation_input.context_id,
                task_id=None  # Let orchestrator generate
            )
            
            # Create protobuf SimulationOutput (PRD requirement)
            proto_output = mantis_core_pb2.SimulationOutput()
            proto_output.context_id = completed_task.context_id
            
            # Copy the A2A Task
            proto_output.simulation_task.CopyFrom(completed_task)
            
            # Set final state from A2A TaskState 
            proto_output.final_state = completed_task.status.state
            
            # Copy response message if available
            if completed_task.history:
                latest_message = completed_task.history[-1]
                proto_output.response_message.CopyFrom(latest_message)
            
            # Copy all artifacts from the task
    ...
    def get_contextual_execution_status(
        self,
        context_id: str
    ) -> List[mantis_core_pb2.SimulationOutput]:
        """
        ...
        """
        logger.debug(
            "Retrieving contextual execution status",
            structured_data={"context_id": context_id}
        )
    ...
    def get_active_contexts(self) -> List[str]:
        ...
        logger.debug(
            ...
            structured_data={"context_count": len(context_ids)}
        )
