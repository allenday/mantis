...
class AbstractTeam(ABC):
    ...
    async def execute_team_member(
        ...
        execution_context = {
            ...
            "assigned_role": member.role_preference if member.role_preference != "UNSPECIFIED" else "FOLLOWER",
        ...
        # Use the original query since AgentInterface doesn't have member_context yet
        # In the future, this could be customized per team member
        member_simulation.query = simulation_input.query
    ...
    async def execute_team(
        ...
        execution_metadata["roles"] = [member.role_preference for member in members]
...
class BaseTeam(AbstractTeam):
    ...
    def _create_team_coordination_context(
        self, member: AgentInterface, team_members: List[AgentInterface], member_index: int, specialized_role: str = ""
    ) -> str:
        """
