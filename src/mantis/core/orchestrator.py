...
class SimulationOrchestrator:
    ...
    def _initialize_tools(self) -> None:
        ...
        try:
            ...
            async def bound_invoke_agent_by_name(agent_name: str, query: str, context: Optional[str] = None, max_depth: int = 1):
                return await invoke_agent_by_name(agent_name, query, self, context, max_depth)
                
            async def bound_invoke_multiple_agents(agent_names: list, query_template: str, individual_contexts: Optional[list] = None, max_depth: int = 1):
    ...
    async def execute_simulation(
        ...
        """
        logger.info("Starting simulation execution", structured_data={
        ...
        try:
            ...
            logger.error("Simulation execution failed", structured_data={
                "context_id": simulation_input.context_id,
                "error": str(e)
            })
    ...
    async def _execute_task_with_agent(
        ...
        if disable_tools:
            logger.info("Executing sub-agent with tools DISABLED", structured_data={
                "context_id": context_id,
                "agent_name": agent_interface.name
            })
        ...
        try:
            ...
            logger.info("Task completed successfully", structured_data={
                "task_id": task_id,
                "agent_name": agent_interface.name,
            ...
            logger.error("Task execution failed", structured_data={
                "task_id": task_id,
                "error": str(e)
            })
            raise
