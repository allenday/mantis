...
@click.option("--format", "-f", type=click.Choice(["table", "json"]), default="table", help="Output format (table=rich table, json=raw JSON)")
        ...
        if query:
            ...
            for agent in agents_data:
                ...
                agent_text = (
                    ...
                    " " + " ".join([skill.get("name", "") + " " + skill.get("description", "") for skill in agent.get("skills", [])]).lower()
                )
