...
coordinate with appropriate agents.""",
        ...
        try:
            ...
            logger.error(
                ...
                structured_data={
                    ...
                    "is_connection_error": any(term in error_msg.lower() for term in ["connection", "timeout", "unreachable"]),
    ...
    def _create_simulation_output(
        ...
        logger.info(
            ...
            structured_data={
                ...
                "has_structured_results_attr": hasattr(self.orchestrator, 'current_structured_results') if self.orchestrator else False,
                "structured_results_count": len(self.orchestrator.current_structured_results) if self.orchestrator and hasattr(self.orchestrator, 'current_structured_results') else 0
            }
        )
        ...
        if self.orchestrator and hasattr(self.orchestrator, 'current_structured_results'):
            ...
            logger.warning(
                ...
                structured_data={
                    "context_id": simulation_input.context_id,
                    "orchestrator_exists": self.orchestrator is not None,
                    "has_structured_results": hasattr(self.orchestrator, 'current_structured_results') if self.orchestrator else False
                }
            )
    ...
    def _convert_tools_to_pydantic_format(self) -> list[Any]:
        ...
        def create_team_formation_wrapper(func):
            async def team_formation_wrapper(count: int = 3) -> str:
                ...
                try:
                    ...
                    result_lines.append(f"✅ **Team Assembly Complete**: {len(agents_list)} agents ready for coordination")
                    
                    return "\n".join(result_lines)
                    
                except Exception as e:
                    # FAIL HARD - team formation errors should be observable and re-raised
                    logger.error("🔥 FAIL HARD: Team formation wrapper - propagating raw error", 
        ...
        def create_invoke_agent_by_url_wrapper(func):
            async def invoke_agent_by_url_wrapper(agent_url: str, query: str, agent_name: Optional[str] = None, context: Optional[str] = None) -> str:
                ...
                try:
                    ...
                    elif hasattr(result, 'response_message') and result.response_message and result.response_message.content:
                    ...
                    logger.error("🔥 FAIL HARD: Direct agent invocation wrapper - propagating raw error", 
        ...
        for tool_name, tool_func in self.tools.items():
            try:
                if tool_name == "web_fetch_url":
                    ...
                    logger.warning("Unknown tool type ignored", 
                ...
                logger.error("Tool conversion failed", 
                           structured_data={"tool_name": tool_name, "error": str(e)})
