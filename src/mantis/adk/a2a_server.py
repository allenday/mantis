...
class ADKA2AServer:
    ...
    def __init__(self, agent_name: str, description: str, port: int = 9053, leader_instruction: str = None):
    ...
    def _initialize_adk_agent(self):
        ...
        try:
            ...
            adk_tools = self._convert_orchestrator_tools_to_adk()
            
            # Use custom leader instruction if provided, otherwise use default
            instruction = self.leader_instruction or f"""You are the {self.agent_name}, a senior coordinator and strategic advisor.
...
work with appropriate agents."""
    ...
    def _convert_orchestrator_tools_to_adk(self):
    ...
    def _create_adk_wrapper(self, orchestrator_tool, tool_name):
        ...
        async def adk_tool_wrapper(*args, tool_context: ToolContext = None, **kwargs):
            """Wrapper that executes orchestrator tool in ADK context."""
            try:
                logger.debug(f"Executing orchestrator tool {tool_name} via ADK wrapper with args={args}, kwargs={kwargs}")
        ...
        adk_tool_wrapper.__signature__ = original_sig
        ...
        try:
            ...
            if simulation_params:
                ...
                simulation_output = await asyncio.wait_for(
                    self.router.route_simulation(simulation_input),
                ...
                response_text = simulation_output.response_message.content[0].text if (simulation_output.response_message and simulation_output.response_message.content) else "No response generated"
                ...
                logger.info(
                    f"🎯 A2A SERVER: ChiefOfStaffRouter returned structured output",
                    structured_data={
                        "task_id": task_id,
                        "context_id": simulation_output.context_id,
                        "structured_results_count": len(simulation_output.results),
                ...
                events = await asyncio.wait_for(
                    self._run_adk_agent_async(user_id, session_id, adk_message),
                    timeout=timeout_seconds
                )
            ...
            if 'response_data' in locals():
                task.result = response_data  # Full structured response with SimulationOutput
            else:
...
def _create_leader_instruction(agent_interface, leader_card):
