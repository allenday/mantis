...
async def git_analyze_repository(repo_url: str) -> str:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            clone_cmd = ["git", "clone", "--depth", "1", "--single-branch", repo_url, str(clone_path)]
            ...
            try:
                branch_result = subprocess.run(
                    ["git", "branch", "--show-current"], cwd=clone_path, capture_output=True, text=True
                )
                if branch_result.returncode == 0:
                    repo_info.append(f"Branch: {branch_result.stdout.strip()}")

                commit_result = subprocess.run(
                    ["git", "rev-parse", "--short", "HEAD"], cwd=clone_path, capture_output=True, text=True
                )
            ...
            try:
                ...
                size_result = subprocess.run(["du", "-sh", str(clone_path)], capture_output=True, text=True)
